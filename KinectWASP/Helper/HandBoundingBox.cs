...
public class HandBoundingBox
{
    ...
    public List<double> FindLocalMaxima(double[] input, int rn,float mu)
    {
        //int windowSize = 2 * rn + 1;
        List<double> maxima = new List<double>();
        for (int i = rn+1; i < input.Length - rn; i++)
        {
            double current = input[i];
            // 1. Monotoniekriterium (Bergform): Werte müssen links steigen und rechts fallen
            ...
            // 2. Mindest-Steigung: Beide Steigungen müssen zusammen größer als mu sein
